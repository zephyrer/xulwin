...
namespace XULWin
{
    ...
    ScrollDecorator::ScrollDecorator(Component * inDecoratedComponent,
        ...
        mOldVerScrollPos(0)
    {
        if (mOverflowX != CSSOverflow_Hidden)
        {
            ...
            mHorizontalScrollbar.reset(new Scrollbar(inDecoratedComponent->parentComponent(), inDecoratedComponent->el()));
            mHorizontalScrollbar->setEventHandler(this);
        }
        if (mOverflowY != CSSOverflow_Hidden)
        {
            AttributesMapping attr;
            attr["orient"] = Orient2String(Vertical);
            mVerticalScrollbar.reset(new Scrollbar(inDecoratedComponent->parentComponent(), inDecoratedComponent->el()));
            mVerticalScrollbar->setEventHandler(this);
        }
    }
